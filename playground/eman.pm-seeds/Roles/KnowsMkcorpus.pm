...
role Roles::KnowsMkcorpus with EmanSeed {
    ...
    method mkcorpus_prepare(Str $corpname) {
        $self->safeSystem($self->corpmanCommand(["--factorindex",
                                                 "--wait",
    ...
    method mkcorpus_corpman_cmd(Str $corpname) {
        return $self->safeBacktick($self->corpmanCommand(['--factorindex',
                                                                   '--cmd',
                                                                   $corpname]));
   }
    ...
    method step_file_info(Str $corpname) {
        my %res;
        my @step_file_col= split(/\s+/, $self->safeBacktick($self->corpmanCommand(["--factorindex", $corpname])));
